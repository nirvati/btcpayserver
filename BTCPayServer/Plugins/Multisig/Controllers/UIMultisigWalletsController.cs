    ...
    IStringLocalizer stringLocalizer) : Controller
{
	...
	private async Task<bool> CanSetupMultisigWallet(StoreData store, string cryptoCode)
	{
		...
		return (await authorizationService.AuthorizeAsync(base.User, store.Id, Policies.CanModifyStoreSettings)).Succeeded;
	}
    ...
    public async Task<IActionResult> SetupMultisig(string storeId, string cryptoCode)
    {
		...
		if (!(await CanSetupMultisigWallet(base.HttpContext.GetStoreData(), vm.CryptoCode)))
			return Forbid();
