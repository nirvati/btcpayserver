...
namespace BTCPayServer.Controllers.Greenfield
{
    ...
    public class GreenfieldStoreUsersController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddOrUpdateStoreUser(string storeId, StoreUserData request, string idOrEmail = null)
        {
            ...
            if (user == null)
                return UserNotFound();
            if (await _userManager.IsInRoleAsync(user, Roles.ServerAdmin) && !(await _authorizationService.AuthorizeAsync(base.User, null, new PolicyRequirement(Policies.CanModifyServerSettings))).Succeeded)
                return this.CreateAPIPermissionError(Policies.CanModifyServerSettings, "Only a server admin can add or update the store membership of a server admin");
        ...
        private async Task<IEnumerable<StoreUserData>> ToAPI(StoreData store)
        {
		    ...
		    bool canManageStoreUsers = (await _authorizationService.AuthorizeAsync(base.User, store.Id, Policies.CanModifyStoreSettings)).Succeeded;
